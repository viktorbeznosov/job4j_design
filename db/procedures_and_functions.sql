...
create or replace procedure delete_where_count_null()
    language 'plpgsql'
as $$
    BEGIN
...
call delete_where_count_null();

create or replace function f_delete_data(u_price integer)
    returns integer
    language 'plpgsql'
as
$$
    declare
        result integer;
    begin
        select into result count(id) from products where price < u_price;
        delete from products where price < u_price;
        return result;
    end;
$$;
